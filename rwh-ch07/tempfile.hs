-- | Another RWH example, with an exhortation to "take a look at the
-- program and see if you can figure out what it does and how it
-- works."  OK!
...
-- | The trickiest part here: how is myAction getting its two
-- arguments?  From the call to "func" in withTempFile.  OK, easy
-- enough.  The "mytemp.txt" is only the pattern passed to
-- openTempFile.  The actual name of the temp file will merely be
...
myAction tempname temph = 
    do -- Start by displaying a greeting on the terminal
       ...
       let tempdata = show [1..10] -- "show" means "to_string", more
                                   -- or less
